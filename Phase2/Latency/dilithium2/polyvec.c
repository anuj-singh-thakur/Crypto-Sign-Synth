...
void polyvecl_uniform_eta(polyvecl *v, const uint8_t seed[SEEDBYTES], uint16_t nonce) {
  ...
  pqcrystals_dilithium2_ref_polyvecl_uniform_eta_label15:for(i = 0; i < L; ++i)
...
void polyvecl_reduce(polyvecl *v) {
  ...
  pqcrystals_dilithium2_ref_polyvecl_reduce_label14:for(i = 0; i < L; ++i)
    poly_reduce(&v->vec[i]);
}
                                       ...
                                       const polyvecl *v)
{
  ...
  pqcrystals_dilithium2_ref_polyvecl_pointwise_acc_montgomery_label1:for(i = 1; i < L; ++i) {
